...
策略模式。在实际的项目开发中，这个模式也比较常用。最常见的应用场景是，利用它来避免冗长的 if-else 或 switch 分支判断。不过，它的作用还不止如此。它也可以提供框架的扩展点等等。
...
使用场景案例: 优惠促销 => 打折、满减、优惠券红包

策略模式也能起到解耦的作用，不过，它解耦的是策略的定义、创建、使用这三部分。接下来，我就详细讲讲一个完整的策略模式应该包含的这三个部分。

### 1. 策略的定义

策略类的定义比较简单，包含一个策略接口和一组实现这个接口的策略类。因为所有的策略类都实现相同的接口，所以，客户端代码**基于接口而非实现编程**，可以灵活地替换不同的策略。示例代码如下所示：
...
实际上，能够移除分支判断逻辑的模式不仅仅有策略模式，后面我们要讲的状态模式也可以。对于使用哪种模式，具体还要看应用场景来定。

> 策略模式适用于根据不同类型的动态，决定使用哪种策略这样一种应用场景。

我们先通过一个例子来看下，`if-else` 或 `switch-case` 分支判断逻辑是如何产生的。
...
上一节课，我们主要介绍了策略模式的原理和实现，以及如何利用策略模式来移除 if-else 或者 switch-case 分支判断逻辑。
...
**每种排序类都是无状态的(没有成员变量)，我们没必要在每次使用的时候，都重新创建一个新的对象**

所以，我们可以使用工厂模式对对象的创建进行封装。按照这个思路，我们对代码进行重构。重构之后的代码如下所示：
...
如果 if-else 分支判断不复杂、代码不多，这并没有任何问题，毕竟 if-else 分支判断几乎是所有编程语言都会提供的语法，存在即有理由。
...
策略模式是定义了一些列算法的方法，从概念上讲，所有这些算法完成的都是相同的工作，只是实现不同
context定义了一系列可供重用的算法或行为
只要在分析过程中，听到需要在不同时间应用不同的业务规则，就可以考虑使用策略模式处理这种变化的可能性。


![image.png](https://cdn.nlark.com/yuque/0/2019/png/313060/1572427529482-3d7fb287-9ebb-4314-a543-e707f6f068cf.png#align=left&display=inline&height=421&name=image.png&originHeight=842&originWidth=1854&size=99037&status=done&width=927)



![image.png](https://cdn.nlark.com/yuque/0/2019/png/313060/1572431997458-f68c992e-3672-4569-bfc5-f4d70ec427f5.png#align=left&display=inline&height=373&name=image.png&originHeight=746&originWidth=1450&size=60448&status=done&width=725)




![image.png](https://cdn.nlark.com/yuque/0/2019/png/313060/1572433960559-4145ffc1-8b30-4f7a-8725-c2b6243c484f.png#align=left&display=inline&height=942&name=image.png&originHeight=1884&originWidth=3296&size=1222339&status=done&width=1648)
...
public class Main {
    ...
    public static void main(String[] args) {
        PriceContext priceContext = new PriceContext("满300减100");
        Double returnResult = priceContext.returnResult(450d);
        System.out.println("实际支付价格为=" + returnResult);//=====>实际支付价格为=350.0
    }
}
