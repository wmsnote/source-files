...
## 2. 泛型基础:hearts::hearts::hearts:

### 2-1. 泛型类:pill::pill::pill:
...
```
这是最常见的做法，这样做的一个坏处是Box里面现在只能装入String类型的元素，今后如果我们需要装入Integer等其他类型的元素，还必须要另外重写一个Box，:sob:
代码得不到复用，使用泛型可以很好的解决这个问题。:smile:
...
```
### 2-2. 泛型方法:satellite::satellite::satellite:

看完了泛型类，接下来我们来了解一下泛型方法。:moneybag::moneybag::moneybag:
声明一个泛型方法很简单，只要在**返回类型前面**加上一个类似`<K, V>`的形式就行了：
```java
public class Util {
...
## 3. 边界符:diamonds::diamonds::diamonds:
...
```
做一个类似于下面这样的声明，这样就等于告诉编译器类型参数T代表的都是实现了`Comparable`接口的类，这样等于告诉编译器它们都至少实现了`compareTo`方法。
```java
...
```
那么现在`Box<Number>  n`允许接受什么类型的参数？我们是否能够传入`Box<Integer>`或者`Box<Double>`呢？
答案是否定的，:cry:
虽然Integer和Double是Number的子类，但是在泛型中`Box<Integer>`或者`Box<Double>`与`Box<Number>`之间并没有任何的关系。:interrobang:
...
```
下面这个例子中，我们创建了一个泛型类Reader，然后在f1()中当我们尝试`Fruit f = fruitReader.readExact(apples);`编译器会报错，因为`List<Fruit>`与`List<Apple>`之间并没有任何的关系。
```java
...
```
答案是否定:cry:
Java编译器不允许我们这样做，为什么呢？:question:
对于这个问题我们不妨从编译器的角度去考虑。因为`List<? extends Fruit> flist`它自身可以有多种含义：
```java
...
当我们尝试add一个Fruit的时候，这个Fruit可以是任何类型的Fruit，而flist可能只想某种特定类型的Fruit，编译器无法识别所以会报错。:100:

所以对于实现了`<? extends T>`的集合类只能将它视为Producer向外提供(get)元素，而不能作为Consumer来对外获取(add)元素。:heart:

如果我们要add元素应该怎么做呢？可以使用`<? super T>`:smile:
```java
...
原因很简单，我们继续从编译器的角度考虑这个问题，对于`List<? super Apple> list`，它可以有下面几种含义：
```java
...
```
这意味着不管我们声明`Node<String>`还是`Node<Integer>`，到了运行期间，JVM统统视为`Node<Object>`。
有没有什么办法可以解决这个问题呢？:question:
...
```
这样编译器就会将T出现的地方替换成`Comparable`而不再是默认的`Object`了：
```java
public class Node {
...
```
假设我们支持泛型数组的创建，由于运行时期类型信息已经被擦除，JVM实际上根本就不知道`new ArrayList<String>()`和`new ArrayList<Integer>()`的区别。
...
所以上面第2行编译器提示unchecked warning的时候，我们不能选择忽略，不然要等到运行期间才能发现异常。
如果我们一开始加上`Node<Integer> n = mn`就好了，这样编译器就可以提前帮我们发现错误。
...
```
实际上对于上面这个问题，还可以采用Factory和Template两种设计模式解决，
感兴趣的朋友不妨去看一下Thinking in Java中第15章中关于Creating instance of types(英文版第664页)的讲解，这里我们就不深入了。
...
```
Part类的实现如下，注意我们上面的实体类都是Part类的间接子类。在Part类我们注册了我们上面的声明的实体类。所以以后我们如果要创建相关的实体类的话，只需要在调用Part类的相关方法了。
这么做的一个好处就是如果的业务中出现了CabinAirFilter或者PowerSteeringBelt的话，我们不需要修改太多的代码，只需要在Part类中将它们注册即可。
```java
class Part {
